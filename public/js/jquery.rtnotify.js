...
(function(jQuery) {
    jQuery.fn.rtnotify = function(options) {
    ...
    var note_area = jQuery("#rtnotify-notification");

    // Construct the new notification.
    var note = jQuery(window.document.createElement('div'))
                .addClass("rtnotify")
    ...
    if (!options['permanent']) {
      note.prepend(jQuery(window.document.createElement('a'))
    ...
    if (options['title'] !== "") {
      note.append(jQuery(window.document.createElement('div'))
    ...
    if (!options['permanent']) {
      if (options['timeout'] != 0) {
        if (options['fade']) {
          ...
          note.queue(function() { jQuery(this).remove(); });
        } else {
          note.delay(options['timeout'] * 1000)
              .queue(function() { jQuery(this).remove(); });
        }
      }
    }
  };
  jQuery.rtnotify = jQuery.fn.rtnotify; // Rename for easier calling.
})(jQuery);
